...
def main():
    ...
    check(
        "local data-plane configuration passes",
        p.get("configuration_gate") == "PASS",
        str(p.get("configuration_findings")),
    )
    db = p.get("database") or {}
    tenant = p.get("tenant_isolation") or {}
    obj = p.get("object_storage") or {}
    backup = p.get("backup_restore") or {}
    ...
    check(
        "transaction posture includes advisory-lock serialization",
        "advisory lock" in str(db.get("transaction_model", "")).lower(),
        str(db),
    )
    ...
    check(
        "tenant-scoped table registry is substantial",
        int(tenant.get("tenant_scoped_table_count", 0)) >= 20,
        str(tenant),
    )
    ...
    o = t.get("object_store") or {}
    ref = o.get("ref") or {}
    verification = o.get("verification") or {}
    check("content-addressed object round-trip verifies", verification.get("ok") is True, str(o))
    check(
        "object verification preserves exact digest",
        bool(ref.get("digest")) and verification.get("actual_digest") == ref.get("digest"),
        str(o),
    )
    sql = t.get("sql_translation") or {}
    check(
        "qmark SQL parameters translate for psycopg",
        sql.get("ok") is True and str(sql.get("sql", "")).count("%s") == 2,
        str(sql),
    )
    ...
    check(
        "request/worker context can switch tenant",
        ((ctx.get("during") or {}).get("tenant_id") == "tenant-self-test"),
        str(ctx),
    )
    check("temporary data-plane context restores cleanly", ctx.get("before") == ctx.get("after"), str(ctx))
    authority = t.get("authority") or {}
    check(
        "data-plane self-test has zero settlement authority",
        authority.get("binding") is False and authority.get("settlement_authority") is False,
        str(authority),
    )
    ...
    check(
        "local online backup can be created",
        st == 200 and b.get("executed") is True and bool(backup_result.get("manifest_file")),
        f"{st} {b}",
    )
    check(
        "backup manifest pins SHA-256 and size",
        backup_result.get("schema") == "arbiter.backup-manifest.v1"
        and len(str(backup_result.get("sha256", ""))) == 64
        and int(backup_result.get("size_bytes", 0)) > 0,
        str(backup_result),
    )
    manifest = backup_result.get("manifest_file")
    st, v, _ = (
        request(a.base_url, "/api/data-plane/backup/verify", "POST", {"manifest_file": manifest}, a.api_key)
        if manifest
        else (0, {}, {})
    )
    check(
        "backup verification passes checksum and SQLite integrity",
        st == 200 and v.get("ok") is True and v.get("sqlite_integrity") == "ok",
        f"{st} {v}",
    )

    st, infra, _ = request(a.base_url, "/api/infrastructure", key=a.api_key)
    check(
        "infrastructure summary includes v0.17 data plane",
        st == 200 and (infra.get("data_plane") or {}).get("version") == "0.17.0",
        f"{st} {infra}",
    )
    st, dev, _ = request(a.base_url, "/api/developer", key=a.api_key)
    check(
        "developer manifest exposes production data plane",
        st == 200
        and (dev.get("production_data_plane") or {}).get("version") == "0.17.0"
        and (dev.get("production_data_plane") or {}).get("tenant_rls") is True,
        f"{st} {dev}",
    )
    ...
    print("PRODUCTION DATA PLANE GATE:", "PASS" if failed == 0 else "FAIL")
