...
def request(base, path, method="GET", body=None, key=None, tenant=None):
    ...
    if tenant:
        headers["X-Arbiter-Tenant"] = tenant
    data = None
    if body is not None:
...
def main():
    ...
    check(
        "API reachable and v0.14+",
        st == 200 and tuple(int(x) for x in str(h.get("version", "0.0")).split(".")[:2]) >= (0, 14),
        f"{st} {h}",
    )
    ...
    tid = "tenant-test-" + suffix
    st, out, _ = request(
        a.base_url,
        "/api/tenants",
        "POST",
        {"tenant_id": tid, "name": "Gate Tenant", "actor": "test:identity-gate"},
        a.api_key,
    )
    ...
    st, out2, _ = request(
        ...
        "/api/tenants",
        "POST",
        {"tenant_id": tid, "name": "Duplicate", "actor": "test:identity-gate"},
        a.api_key,
    )
    ...
    st, pr, _ = request(
        ...
        "POST",
        {
            ...
            "actor": "test:identity-gate",
        },
        a.api_key,
    )
    check(
        "service principal can be registered",
        st == 200 and pr.get("principal", {}).get("principal_type") == "service",
        f"{st} {pr}",
    )
    st, prs, _ = request(a.base_url, f"/api/principals?tenant_id={tid}", key=a.api_key)
    check(
        "principals can be tenant-filtered",
        st == 200 and any(x.get("principal_id") == pid for x in prs.get("principals", [])),
        str(prs),
    )
    st, bad, _ = request(
        a.base_url,
    ...
    check(
        ...
        st == 200 and any((e.get("action") in {"tenant.created", "principal.registered"}) for e in (events or [])),
        str(audit)[:800],
    )
    st, ready, _ = request(a.base_url, "/api/readiness", key=a.api_key)
    check(
        "readiness remains healthy after identity operations", st == 200 and ready.get("ready") is True, f"{st} {ready}"
    )
    total = passed + failed
    print("\n" + "=" * 64)
    print(f"RESULT: {passed}/{total} checks passed; {failed} failed")
    print("IDENTITY & TENANT GATE:", "PASS" if failed == 0 else "FAIL")
