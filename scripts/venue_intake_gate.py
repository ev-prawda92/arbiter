...
IRAN_RULES = (
    "If, before the close date, the United States and Iran sign a formal written agreement that "
...
def kalshi(ticker, title, rules, status="active", result=""):
    return {
        ...
        "settlement_ts": "2026-09-01T15:00:00Z" if result else None,
        "yes_bid": 7,
        "volume": 120345,
    }
...
def poly(mid, question, description, closed=False, prices=("0.42", "0.58"), uma="", disputes=0):
    return {
        ...
        "umaResolutionStatuses": json.dumps(["proposed"] + ["disputed"] * disputes),
        "closedTime": "2026-01-01T08:00:00Z" if closed else None,
        "umaEndDate": None,
        "volume": "88123.4",
    }
...
FIXTURES = {
    ("kalshi", "KXIRANDEAL-26SEP"): kalshi(
        "KXIRANDEAL-26SEP", "Will the US agree to a new Iranian nuclear deal?", IRAN_RULES
    ),
    ("kalshi", "KXIRANDEAL-26AUG"): kalshi(
        "KXIRANDEAL-26AUG", "Will the US agree to a new Iranian nuclear deal?", IRAN_RULES
    ),
    ("polymarket", "900001"): poly(
        "900001",
    ...
    ("kalshi", "KXBTC-100K"): kalshi(
        "KXBTC-100K",
        "Bitcoin above $100,000 on Dec 31?",
        "Resolves Yes if the Bitcoin price is above $100,000 at 23:59 ET on Dec 31.",
    ),
    ("polymarket", "900002"): poly(
        "900002", "US CPI above 3.0% in August?", "Resolves per the BLS CPI release for August."
    ),
    ("kalshi", "KXSHUTDOWN-Q4"): kalshi(
        "KXSHUTDOWN-Q4",
        "Government shutdown in Q4?",
        "Resolves Yes if a lapse in appropriations begins.",
        status="settled",
        result="yes",
    ),
    ("polymarket", "900003"): poly(
        "900003",
        "Government shutdown in Q4?",
        "Resolves Yes if a lapse in appropriations begins.",
        closed=True,
...
WATCHLIST = [
    {
        "seed_event_id": "iran-deal",
        "label": "US-Iran nuclear deal",
        "review_class": "interpretive_criteria",
        "kalshi": ["KXIRANDEAL-26SEP", "KXIRANDEAL-26AUG"],
        "polymarket": ["900001"],
    },
...
def main() -> None:
    ...
    check(
        audit_count(dry_store) == 0 and dry_store.summary()["contracts"] == 0,
    ...
    # V8
    check(
        len(report["errors"]) == 1 and report["errors"][0]["market_id"] == "999999",
    ...
    # V1
    check(
        by[("kalshi", "KXIRANDEAL-26SEP")]["work_item"] == "opened"
        and by[("kalshi", "KXIRANDEAL-26AUG")]["work_item"] == "opened",
        "V1 both open Iran markets opened work items",
    )
    check(
        by[("polymarket", "900002")]["review_class"] == "revised_source"
    ...
    check(
        iran_cluster["count"] == 2 and iran_cluster["blocker_type"] == "policy_interpretation",
    ...
    check(
        btc["work_item"] == "none" and btc["contract_id"] and btc["evidence_id"],
    ...
    ev = next(
        e
        for e in store.list_evidence("kalshi:KXIRANDEAL-26SEP")
        if e["evidence_id"] == by[("kalshi", "KXIRANDEAL-26SEP")]["evidence_id"]
    )
    check(
        ev["raw_payload_hash"] == canonical_hash(FIXTURES[("kalshi", "KXIRANDEAL-26SEP")]),
        "V3 evidence raw_payload_hash is the hash of the full raw venue response",
    )

    # V9
    check(
        report["totals"]["cross_venue_disagreements"] == 1, "V9 shutdown event detected as a cross-venue disagreement"
    )
    ...
    check(
        again["totals"]["evidence_appended"] == 0 and again["totals"]["work_items_opened"] == 0,
        "V4 re-sync with only price changes appends no evidence and opens nothing",
    )
    check(
        audit_count(store) == before_audit and store.summary()["evidence_records"] == before_counts["evidence_records"],
    ...
    svc.create(
        ...
        question="Does the MOU count as a deal?",
        selection="MOU is non-qualifying",
        rationale="No verifiable restrictions or sanctions relief.",
        actor="operator:gate",
        affected_case_ids=list(iran_cluster["case_ids"]),
    ...
    # V6
    fixtures[("polymarket", "900002")].update(
        closed=True, outcomePrices=json.dumps(["1", "0"]), umaResolutionStatus="resolved"
    )
    ...
    check(
        cpi["status"] == "settled" and cpi["outcome"] == "YES" and cpi["evidence_id"],
        "V6 settlement appends new evidence",
    )
    ...
    check(
        newest["revision_number"] == 2 and newest["supersedes"],
        "V6 the new evidence supersedes the open-market evidence",
    )
    ...
    check(
        state["status"] == "resolved" and "settled YES" in state["note"], "V6 the closed item carries the venue outcome"
    )
    ...
    check(
        btc2["contract_version"] == 2 and btc2["contract_version_created"], "V7 rules change creates contract version 2"
    )
    ...
    listings = {
        "kalshi": [
            ...
            (
                kalshi("KXBTC-120K", "Bitcoin above $120,000?", "Resolves Yes if Bitcoin is above $120,000 on Dec 31."),
                "fixture://k/3",
            ),
            ...
            *[
                (
                    kalshi(
                        f"KXWNBA{q}QWINNER-26SEP23DALSEA-SEA",
                        f"Will Seattle win the {q}th quarter?",
            ...
            *[
                (
                    kalshi(
                        f"KXETH-{n}K",
                        f"Ethereum above ${n},000?",
                        f"Resolves Yes if Ethereum is above ${n},000 on Dec 31.",
                    ),
                    f"fixture://k/e{n}",
                )
                for n in range(3, 9)
        ...
        "polymarket": [
            (
                dict(
                    poly("910001", "CPI above 3% in October?", "Resolves per the BLS CPI release for October."),
                    events=[{"id": "e77", "title": "US CPI October"}],
                ),
                "fixture://p/1",
            ),
            ...
            *[
                (
                    dict(
                        poly(
                            f"92000{n}",
                            f"Will team {n} make the playoffs?",
                        ...
                        events=[{"id": f"s{n}", "title": f"Team {n} playoffs"}],
                    ),
                    f"fixture://p/s{n}",
                )
                for n in range(1, 10)
    ...
    check(
        keys.get("kalshi-KXCEASE-26") == ["KXCEASE-26-NOV", "KXCEASE-26-OCT"],
        "V11 the two ceasefire markets are grouped under one Kalshi event",
    )
    check(
        "polymarket-e77" in keys
        and not any("BTC" in k or "ETH" in k or "MVE" in k or "DONE" in k for ks in keys.values() for k in ks),
        "V11 CPI kept; clean threshold, parlay and settled markets skipped",
    )
    check(
        not any("WNBA" in k for ks in keys.values() for k in ks),
        "V12 sports 'win the quarter' markets are not flagged as contested official sources",
    )
    check(
        not any(k.startswith("polymarket-s") for k in keys),
        "V12 Polymarket's template 'consensus of credible reporting' alone does not flag a market",
    )
    check(
        "consensus of credible reporting" in stats["boilerplate"]["polymarket"],
    ...
    check(
        drep["totals"]["work_items_opened"] == 3 and len(cease) == 1,
    ...
    def pitcher(ticker, name, stat, n):
        return kalshi(
            ...
            "rules_secondary": (
                f"Player Participation & Settlement Criteria: If {name} is scratched or is not a starting "
    ...
    mou = (
        "If the US and Iran sign a formal agreement with verifiable restrictions and sanctions relief, resolves Yes. "
    ...
    pool = [
        ...
        *[
            (
                kalshi(
                    f"KXETH-{n}K", f"Ethereum above ${n},000?", f"Resolves Yes if Ethereum is above ${n},000 on Dec 31."
                )
            )
            for n in range(3, 9)
        ],
    ]
    ev15, _, st15 = venue_intake.discover(
        lambda venue, limit: [(m, "fixture") for m in pool] if venue == "kalshi" else [], limit=50
    )
    got15 = sorted(m for e in ev15 for _, m in e.markets)
    check(
        not any("KXMLB" in m for m in got15), "V15 pitcher props sharing fine print across two events are not flagged"
    )
    check(
    ...
    check(
        not any(
            t.startswith(("KXNEXTDNCCHAIR", "KXNEXTNATOSECGEN", "KXNEXTROMANIAPM", "KXAFRICALEADEROUT")) for t in flat24
        ),
            ...
            ev(
                ...
                [
                    ...
                    nested(
                        ...
                        "Fed cuts 25bp?",
                        "Resolves Yes if the target range is cut by 25 bp.",
                        status="closed",
                    ),
                ],
            ),
    ...
    check(
        ...
        == [
            ...
            "KXNEXTNATOSECGEN-99-KIOH",
            "KXNEXTNATOSECGEN-99-RUTT",
    ...
    check(
        [m["ticker"] for m, _, _ in only] == ["KXFEDDECISION-26OCT-H0"],
        "V16 category allow-list honoured (case-insensitive)",
    )
    ...
    check(
        len(nato) == 1 and len(nato[0].markets) == 3 and nato[0].label.startswith("Who will be the next Secretary"),
        "V16 the NATO event's one-event clarification is flagged as ONE pattern, labelled with the event title",
    )
    check(
        kst["by_category"]["kalshi"]
        == {"Elections": {"scanned": 3, "flagged": 3}, "Economics": {"scanned": 1, "flagged": 0}},
    ...
    DecisionRecordService(tstore).create(
        decision_type="policy_interpretation",
        question="q",
        selection="s",
        rationale="r",
        actor="operator:gate",
        affected_case_ids=[decided_item["work_item_id"]],
        cluster_id="c",
    )
    pinned_events = [
        venue_intake.WatchedEvent(
            event_id="pinned",
            label="pinned",
            review_class="interpretive_criteria",
    ...
    check(
        len(evsvc.list_exceptions()) == open_before and audit_count(tstore) == audit_before,
        "V14 dry-run re-triage writes nothing",
    )
    tri = venue_intake.retriage(tstore)
    check(
        tri["totals"]["closed"] == preview["totals"]["closed"] > 0,
        f"V14 re-triage closed {tri['totals']['closed']} stale items",
    )
    ...
    remaining = [
        x
        for x in evsvc.list_exceptions()
    ...
    check(
        got_remaining == sorted(set(flagged) - held),
    ...
    tstore.set_work_state(
        evsvc.get_exception(op_id)["work_item_id"], "resolved", "Evan", "checked, fine", "operator:evan"
    )
    ...
    check(
        any("Closed by re-triage" in (v.get("note") or "") for v in closed_state.values()),
