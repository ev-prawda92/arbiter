...
def request(base, path, method="GET", body=None, key=None):
    ...
    try:
        with urllib.request.urlopen(req, timeout=10) as r:
            raw = r.read()
            return r.status, (json.loads(raw.decode()) if raw else {}), dict(r.headers)
...
def main():
    ...
    try:
        ver = tuple(int(x) for x in str(h.get("version", "0.0")).split(".")[:2])
    except Exception:
        ver = (0, 0)
    ...
    check(
        "reliability schema migration recorded",
        any(m.get("migration_id") == "v0.15-reliability" for m in p.get("migrations", [])),
        str(p),
    )
    ...
    check(
        "same idempotency request replays deterministically",
        st == 200 and i2.get("replayed") is True and i2.get("response") == {"ok": True},
        f"{st} {i2}",
    )
    bad = dict(idem)
    bad["request"] = {"x": 2}
    ...
    st, j, _ = request(
        ...
        "POST",
        {
            "job_type": "resolution.reevaluate",
            "payload": {"contract_id": "C-" + suffix},
            "tenant_id": "local",
            "max_attempts": 3,
            "actor": "test:reliability-gate",
        },
        a.api_key,
    )
    job = j.get("job", {})
    ...
    st, c, _ = request(
        ...
        "/api/jobs/claim",
        "POST",
        {"worker_id": "worker:A", "tenant_id": "local", "lease_seconds": 60},
        a.api_key,
    )
    claimed = c.get("job") or {}
    check(
        "worker can claim due job",
        st == 200 and claimed.get("job_id") == jid and claimed.get("status") == "running",
        f"{st} {c}",
    )
    check("claim increments attempt count", claimed.get("attempts") == 1, str(claimed))
    st, w, _ = request(
        a.base_url, f"/api/jobs/{jid}/complete", "POST", {"worker_id": "worker:B", "result": {}}, a.api_key
    )
    ...
    st, d, _ = request(
        ...
        "POST",
        {"worker_id": "worker:A", "result": {"outcome": "ok"}},
        a.api_key,
    )
    ...
    st, c2, _ = request(
        ...
        "/api/jobs/claim",
        "POST",
        {"worker_id": "worker:DLQ", "tenant_id": "local", "lease_seconds": 60},
        a.api_key,
    )
    check("terminal-test job claimed", st == 200 and (c2.get("job") or {}).get("job_id") == jid2, f"{st} {c2}")
    st, f2, _ = request(
        a.base_url,
        f"/api/jobs/{jid2}/fail",
        "POST",
        {"worker_id": "worker:DLQ", "error": "simulated permanent failure", "retry_delay_seconds": 1},
        a.api_key,
    )
    check(
        "max-attempt failure moves job to dead letter",
        st == 200 and f2.get("job", {}).get("status") == "dead_letter",
        f"{st} {f2}",
    )
    check(
        "dead-letter retains failure reason",
        "simulated permanent failure" in (f2.get("job", {}).get("last_error") or ""),
        str(f2),
    )
    ...
    check(
        "completed and dead-letter states persist",
        any(x.get("job_id") == jid and x.get("status") == "completed" for x in jobs)
        and any(x.get("job_id") == jid2 and x.get("status") == "dead_letter" for x in jobs),
        str(jobs)[:800],
    )
    ...
    st, wh, _ = request(
        ...
        "POST",
        {
            ...
            "target_url": "http://localhost/test-hook",
            "payload": {"run_id": "run-" + suffix, "outcome": "YES"},
            "tenant_id": "local",
            "max_attempts": 3,
            "actor": "test:reliability-gate",
        },
    ...
    check(
        "signed webhook envelope can be created",
        st == 200 and str(delivery.get("signature", "")).startswith("sha256=") and bool(delivery.get("nonce")),
        f"{st} {wh}",
    )
    verify = {
    ...
    check(
        ...
        st == 200 and v2.get("valid") is False and v2.get("reason") == "replay detected",
        f"{st} {v2}",
    )
    badsig = dict(verify)
    badsig["nonce"] = "new-" + suffix
    badsig["signature"] = "sha256:" + "0" * 64
    st, v3, _ = request(a.base_url, "/api/webhook-verify", "POST", badsig, a.api_key)
    check(
        "invalid webhook signature is rejected",
        st == 200 and v3.get("valid") is False and v3.get("reason") == "signature mismatch",
        f"{st} {v3}",
    )

    st, p2, _ = request(a.base_url, "/api/reliability", key=a.api_key)
    check(
        "reliability posture exposes dead letters", st == 200 and p2.get("jobs", {}).get("dead_letter", 0) >= 1, str(p2)
    )
    ...
    check(
        "reliability operations are audited",
        st == 200
        and {"job.enqueued", "job.claimed", "job.completed", "job.dead_lettered", "webhook.queued"}.issubset(actions),
        str(actions),
    )
    ...
    check(
        "readiness remains healthy after reliability operations",
        st == 200 and ready.get("ready") is True,
        f"{st} {ready}",
    )
    ...
    print("PRODUCTION RELIABILITY GATE:", "PASS" if failed == 0 else "FAIL")
