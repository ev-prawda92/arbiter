...
def main():
    ...
    check(
        "API reachable and v0.18+",
        st == 200 and tuple(int(x) for x in str(h.get("version", "0.0")).split(".")[:2]) >= (0, 18),
        f"{st} {h}",
    )
    ...
    check(
        "model provider admin registry loads",
        st == 200 and isinstance(providers.get("providers"), list),
        f"{st} {providers}",
    )
    ...
    check(
        "OIDC signing algorithms restricted",
        fed.get("algorithms") == ["RS256"] or bool(fed.get("algorithms")),
        str(fed),
    )
    ...
    check(
        "infrastructure includes enterprise secrets",
        st == 200 and (infra.get("enterprise_secrets") or {}).get("version") == "0.18.0",
        f"{st} {infra}"[:1400],
    )
    check(
        "infrastructure includes identity federation",
        st == 200 and (infra.get("identity_federation") or {}).get("version") == "0.18.0",
        f"{st} {infra}"[:1400],
    )

    st, dev, _ = request(a.base_url, "/api/developer", key=a.api_key)
    check(
        "developer manifest exposes enterprise secrets",
        (dev.get("enterprise_secrets") or {}).get("write_only_secret_api") is True,
        str(dev)[:1400],
    )
    check(
        "developer manifest exposes OIDC federation",
        (dev.get("identity_federation") or {}).get("oidc_bearer") is True,
        str(dev)[:1400],
    )
    ...
    check(
        "secret administration writes audit events",
        st == 200 and any(e.get("action") in {"model.provider.configured", "model.provider.disabled"} for e in events),
        str(events)[:1200],
    )
    check(
        "audit chain remains valid after secret operations",
    ...
    print("ENTERPRISE SECRETS & FEDERATION GATE:", "PASS" if failed == 0 else "FAIL")
