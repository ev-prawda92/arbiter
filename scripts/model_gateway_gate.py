...
def main():
    ...
    check(
        "API reachable and v0.16+",
        st == 200 and tuple(int(x) for x in str(h.get("version", "0.0")).split(".")[:2]) >= (0, 16),
        f"{st} {h}",
    )
    ...
    check(
        "self-test has zero settlement authority",
        authority.get("binding") is False and authority.get("settlement_authority") is False,
        str(authority),
    )
    prov = selft.get("provenance") or {}
    check(
        "self-test records prompt identity/version",
        bool(prov.get("prompt_id")) and bool(prov.get("prompt_version")),
        str(prov),
    )
    check(
        "self-test records input and output hashes",
        bool(prov.get("input_hash")) and bool(prov.get("output_hash")),
        str(prov),
    )
    check("self-test structured validation passes", prov.get("validation_status") == "PASS", str(prov))
    check(
        "self-test output labels itself advisory",
        (selft.get("output") or {}).get("advisory_only") is True,
        str(selft.get("output")),
    )
    ...
    check(
        "registry persists non-binding metadata",
        bool(row) and (row.get("metadata") or {}).get("binding") is False,
        str(row),
    )

    st, infra, _ = request(a.base_url, "/api/infrastructure", key=a.api_key)
    check(
        "infrastructure posture includes model gateway",
        st == 200 and (infra.get("model_gateway") or {}).get("gateway_version") == "0.16.0",
        f"{st} {infra}",
    )

    st, dev, _ = request(a.base_url, "/api/developer", key=a.api_key)
    check(
        "developer manifest exposes model intelligence",
        st == 200 and (dev.get("model_intelligence_gateway") or {}).get("settlement_authority") is False,
        f"{st} {dev}",
    )
    ...
    check(
        "model invocation is written to audit chain",
        st == 200
        and any(e.get("action") == "model.invocation.recorded" and e.get("object_id") == inv_id for e in events),
        str(events)[:1200],
    )
    check(
        "audit chain remains valid after model self-test",
        (audit.get("chain") or {}).get("ok") is True,
        str(audit.get("chain")),
    )
    ...
    print("MODEL INTELLIGENCE GATE:", "PASS" if failed == 0 else "FAIL")
