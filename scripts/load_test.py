...
def main():
    ...
    ap.add_argument("--path", default="/api/readiness")
    ap.add_argument("--requests", type=int, default=200)
    ap.add_argument("--concurrency", type=int, default=20)
    ap.add_argument("--api-key")
    ap.add_argument("--timeout", type=float, default=10)
    ap.add_argument("--out", default="arbiter_load_result.json")
    a = ap.parse_args()
    headers = {}
    if a.api_key:
        headers["X-Arbiter-Key"] = a.api_key
    n = max(1, a.requests)
    c = max(1, min(a.concurrency, n))
    started = datetime.now(timezone.utc).isoformat()
    t0 = time.perf_counter()
    with concurrent.futures.ThreadPoolExecutor(max_workers=c) as ex:
        results = list(ex.map(lambda _: once(a.base_url.rstrip("/") + a.path, headers, a.timeout), range(n)))
    elapsed = time.perf_counter() - t0
    lats = [r["latency_ms"] for r in results]
    ok = [r for r in results if r["status"] and 200 <= r["status"] < 300]
    report = {
        "tool": "arbiter-load-probe-v1",
        "base_url": a.base_url,
        "path": a.path,
        "requests": n,
        "concurrency": c,
        ...
        "requests_per_second": round(n / max(elapsed, 1e-9), 2),
        ...
        "errors": {
            e: sum(1 for r in results if r["error"] == e) for e in sorted({r["error"] for r in results if r["error"]})
        },
    }
    payload = json.dumps(report, sort_keys=True, separators=(",", ":")).encode()
    report["evidence_sha256"] = hashlib.sha256(payload).hexdigest()
    open(a.out, "w").write(json.dumps(report, indent=2, sort_keys=True) + "\n")
