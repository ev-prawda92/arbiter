...
def main():
    ...
    st, h = get("/api/health")
    check(
        "API reachable and v0.13",
        st == 200 and tuple(int(x) for x in str(h.get("version", "0.0")).split(".")[:2]) >= (0, 13),
        f"{st} {h}",
    )
    check(
        "product identifies semantic contract intelligence",
        "Semantic Contract Intelligence" in str(h.get("product", "")),
        str(h),
    )
    ...
    check(
        "CPI explained in plain language",
        "price index" in v.get("concepts", [{}])[0].get("plain_language", "").lower(),
        str(v),
    )
    ...
    check(
        "publisher extracted",
        v.get("extracted_semantics", {}).get("publisher") == "U.S. Bureau of Labor Statistics",
        str(v),
    )
    check(
        "revision semantics extracted",
        v.get("extracted_semantics", {}).get("revision_semantics") == "first_release",
        str(v),
    )
    check("semantic analysis has stable hash", str(v.get("semantic_hash", "")).startswith("sha256:"), str(v))
    check(
        "semantic layer states non-settlement boundary",
        "does not itself determine settlement" in v.get("boundary", ""),
        str(v),
    )
    ...
    st, p = post(
        ...
        {**base, "contract_id": "SEM-" + uuid.uuid4().hex[:8], "title": precise_title, "rules": precise_rules},
    )
    ...
    st, f = post(
        "/api/semantic-analyze",
        {**base, "contract_id": "SEM-" + uuid.uuid4().hex[:8], "title": fed_title, "rules": fed_rules},
    )
    check(
        "Fed concept recognized",
        st == 200 and f.get("concepts", [{}])[0].get("concept_id") == "MONETARY_POLICY.FOMC_RATE_ACTION",
        str(f),
    )
    check(
        "Fed target range extracted",
        f.get("extracted_semantics", {}).get("rate_instrument") == "federal_funds_target_range",
        str(f),
    )
    check(
        "Fed governing publication extracted",
        f.get("extracted_semantics", {}).get("governing_publication") == "FOMC statement",
        str(f),
    )
    ...
    st, c = post(
        "/api/compile",
        {**base, "contract_id": "SEM-" + uuid.uuid4().hex[:8], "title": vague_title, "rules": vague_rules},
    )
    ...
    check(
        "developer manifest exposes semantic endpoint",
        st == 200 and d.get("semantic_contract_intelligence", {}).get("endpoint") == "/api/semantic-analyze",
        str(d),
    )
