...
def candidates() -> list[dict]:
    ...
    for i in range(8):
        ...
        if i == 0:
            row["rules"] = (
                "This fixture resolves YES if the Bureau of Labor Statistics reports CPI above 10 on September 10, 2026 at 12:00 UTC."
            )
...
def main() -> None:
    ...
    try:
        ...
        for name in (
            "v0.27 benchmark posture endpoint loads",
            "real holdout target is explicit",
            "production certification remains false",
            "holdout tuning is prohibited",
        ):
            chk(name, False)
    ...
    kraw = {
        ...
        "rules_primary": "Resolves YES if the Bureau of Labor Statistics reports CPI above 3.0% on September 11, 2026 at 08:30 EDT. The first published release controls.",
        "rules_secondary": "Official BLS data governs.",
        "result": "yes",
        "settlement_ts": "2026-09-11T13:00:00Z",
        "market_type": "binary",
    }
    ...
    praw = {
        "id": "123",
        "question": "Will official total exceed 10?",
        "description": "This market resolves YES if official results show a total above 10 by September 12, 2026 at 12:00 UTC.",
        "resolutionSource": "Official results",
        "outcomes": '["Yes","No"]',
        "outcomePrices": '["1","0"]',
        "closed": True,
        "umaResolutionStatus": "resolved",
        "closedTime": "2026-09-12T13:00:00Z",
    }
    ...
    chk(
        "Polymarket nonterminal price is rejected",
        normalize_polymarket_market({**praw, "outcomePrices": '["0.7","0.3"]'}, "https://example.test/poly") is None,
    )
    ...
    with tempfile.TemporaryDirectory(prefix="arbiter-real-bench-") as td:
        ...
        chk(
            "holdout aggregate hash is pinned",
            str(frozen.get("hashes", {}).get("dataset_sha256", "")).startswith("sha256:"),
        )
        chk(
            "labels are declared separated",
            frozen.get("benchmark_controls", {}).get("labels_separated_from_inputs") is True,
        )
        chk(
            "frozen evidence is separately hash pinned",
            str(frozen.get("hashes", {}).get("evidence_sha256", "")).startswith("sha256:"),
        )
        ...
        chk(
            "evidence contains no venue outcome labels",
            all("known_outcome" not in x and "settlement_value" not in x for x in evidence),
        )
        ...
        chk(
            "blind predictions are hash pinned",
            str(blind.get("manifest", {}).get("predictions_sha256", "")).startswith("sha256:"),
        )
        chk(
            "blind run pins the frozen evidence hash",
            blind.get("manifest", {}).get("evidence_sha256") == frozen.get("hashes", {}).get("evidence_sha256"),
        )
        preds = read_jsonl(run_dir / "predictions.jsonl")
        chk(
            "blind prediction rows contain no venue labels",
            all("known_outcome" not in x and "settlement_value" not in x for x in preds),
        )
        p1 = next((x for x in preds if x.get("case_id") == "fixture-1"), {})
        chk(
            "READY numeric contract resolves from frozen evidence",
            p1.get("predicted_outcome") == "YES" and p1.get("resolution_method") == "numeric-threshold-v1",
        )
        chk(
            "non-READY contracts abstain with HOLD",
            all(x.get("predicted_outcome") == "HOLD" for x in preds if x.get("case_id") != "fixture-1"),
        )
        ...
        chk(
            "post-run report is hash pinned",
            str(report.get("report", {}).get("report_sha256", "")).startswith("sha256:"),
        )
        oq = report.get("report", {}).get("metrics", {}).get("outcome_metrics", {})
        chk(
            "report scores only evidence-backed deterministic outcomes",
            oq.get("scored_cases") == 1 and oq.get("agreement") == 1.0,
        )
        chk("report exposes governed hold rate", oq.get("held_cases") == 5 and oq.get("hold_rate") is not None)
        chk(
            "report exposes gold coverage separately",
            report.get("report", {}).get("metrics", {}).get("gold_contract_quality", {}).get("gold_status_coverage")
            == 6,
        )

        # Tamper detection after a completed benchmark.
        with (d1 / "labels.jsonl").open("a", encoding="utf-8") as f:
            f.write('{"tamper":true}\n')
        tampered = verify_dataset(d1)
        chk(
            "label tampering invalidates dataset",
            tampered.get("ok") is False and any("hash mismatch" in x for x in tampered.get("errors", [])),
        )
