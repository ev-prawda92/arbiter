...
def exception(subject: str, title: str, detail: str, action: str, kind: str, severity: str = "high") -> dict:
    ...
    return {"kind": kind, "subject": subject, "title": title, "detail": detail,
...
def build_queue(store: ResolutionStore) -> dict:
    ...
    infra = {
        ...
        "audit_chain": {"ok": True},
        "evidence_exceptions": [
            exception("KXUSAIRANAGREEMENT-27-26SEP",
                      ...
                      MOU_ACTION, "policy_review"),
            exception("KXUSAIRANAGREEMENT-27-26AUG",
                      "Interpretive: does the June MOU count as a deal? (Kalshi 26AUG)",
                      "Kalshi requires a formal written instrument with verifiable restrictions and sanctions relief.",
                      MOU_ACTION, "policy_review"),
            exception("POLYMARKET-521878",
                      ...
                      MOU_ACTION, "policy_review"),
            exception("POLYMARKET-527838",
                      "Which CPI print controls? (March 2025, Polymarket 527838)",
                      "Initial BLS release vs later revision.",
                      "Confirm whether the initial or revised CPI release controls.", "timing_revision", "medium"),
            exception("POLYMARKET-546814",
                      "Interpretive: does the outfit count as a suit? (Polymarket 546814)",
                      "Disputed through five UMA rounds.",
                      "Record whether the outfit worn qualifies as a suit under the market rules.", "policy_review", "medium"),
        ],
    }
...
def workload(queue: dict) -> dict:
    ...
    return {"active_cases": s["active_cases"], "human_decisions": s["estimated_human_decisions"],
            "work_patterns": s["distinct_work_patterns"]}


def compact(queue: dict) -> list[dict]:
    return [{"id": i["id"], "subject": i["subject"], "status": i["status"],
             "note": i.get("note") or "", "governed_by": i.get("governed_by")} for i in queue["items"]]
...
def main() -> None:
    ...
    for aid, name, org, kind in [
        ...
        store.save_authority(Authority(authority_id=aid, version=1, name=name, organization=org,
                                       source_type=kind), actor="system:intake")
    contracts = {}
    ...
    def evidence(authority_id: str, contract_id: str, value: dict, locator: str, observed: str) -> dict:
        rec = EvidenceRecord(evidence_id=gen_id("evid"), authority_id=authority_id, authority_version=1,
                             ...
                             raw_payload_hash=canonical_hash(value), parser_version="demo-0.2",
                             contract_id=contract_id, source_locator=locator)
        return store.append_evidence(rec, actor="system:evidence")

    e_mou = evidence("auth_official_statements", contracts["kalshi"],
    ...
    store.set_work_state(pm_item["id"], "resolved", "Resolution Ops",
                         "Settled NO on 2026-01-01; window closed before the MOU.", "system:venue-sync")
    ...
    decision = svc.create(
        ...
        rationale=("The MOU maintained the status quo and deferred substantive commitments to future "
    ...
    application = apply_decision_to_workflow(resolution_store=store, decision=decision,
                                             current_queue=queue_before, actor=ACTOR)
    ...
    reversal = wsvc.create(
        ...
        actor=ACTOR, affected_case_ids=list(cluster["case_ids"]), cluster_id=cluster["cluster_id"],
        contract_id=contracts["kalshi"], supersedes=decision["decision_id"],
        metadata={"what_if": True},
    )
    authoritative_ids = [d["decision_id"] for d in wsvc.authoritative(limit=500)]
    try:
        apply_decision_to_workflow(resolution_store=wstore, decision=decision,
                                   current_queue=build_queue(wstore), actor=ACTOR)
        old_refused = False
    except ValueError:
        old_refused = True
    ...
    whatif = {
        ...
        "cases_now_cite": sorted({(i.get("governed_by") or {}).get("decision_id") for i in wqueue["items"]
                                  if i.get("governed_by")}),
        "audit_ok": wstore.verify_audit_chain(),
    }

    cleared = [i for i in queue_after["items"] if (i.get("governed_by") or {}).get("decision_id") == decision["decision_id"]]
    result = {
        ...
        "work_patterns_before": [{"cluster_id": c["cluster_id"], "blocker_type": c["blocker_type"],
                                  "count": c["count"], "case_ids": c["case_ids"]} for c in ops["clusters"]],
        "iran_pattern": {"cluster_id": cluster["cluster_id"], "blocker_type": cluster["blocker_type"],
                         "case_ids": cluster["case_ids"], "question": prompt["question"]},
        "decision": decision,
        "application": application,
        "queue_after": compact(queue_after),
        "workload": {"before": before, "after": after,
    ...
    print(json.dumps({
        ...
        "semantic_counts_unchanged": all(counts_before[k] == counts_after[k]
                                         for k in ("contracts", "authorities", "evidence_records", "resolution_runs")),
        "chain": chain, "tamper": result["tamper_test"],
        "what_if": {k: whatif[k] for k in ("original_is_authoritative", "original_refused_at_apply",
                                           "records_kept", "cases_now_cite")},
    }, indent=2))
