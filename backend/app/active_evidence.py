...
class ActiveEvidenceService:
    ...
    def list_monitors(self, contract_id: str | None = None) -> list[dict[str, Any]]:
        with self.store.connect() as db:
            if contract_id:
                rows = db.execute(
                    "SELECT * FROM evidence_monitors WHERE contract_id=? ORDER BY created_at DESC", (contract_id,)
                ).fetchall()
            else:
    ...
    def update_monitor(
        ...
        *,
        ...
        with self.store.connect() as db:
            db.execute(
                "UPDATE evidence_monitors SET config_json=?,enabled=?,schedule_seconds=?,updated_at=? WHERE monitor_id=?",
                (_json(cfg), 1 if enabled_value else 0, schedule, now, monitor_id),
            )
        self.store._audit(
            actor,
            "evidence.monitor.updated",
            "evidence_monitor",
            monitor_id,
    ...
    def get_exception(self, exception_id: str) -> dict[str, Any] | None:
        ...
        d["work_item_id"] = _work_id(d["kind"], d["subject"])
        return d
    ...
    def list_exceptions(self, active_only: bool = True) -> list[dict[str, Any]]:
        with self.store.connect() as db:
            if active_only:
                rows = db.execute(
                    "SELECT * FROM evidence_exceptions WHERE status<>'resolved' ORDER BY updated_at DESC"
                ).fetchall()
            else:
    ...
    def _request_reevaluation(self, contract_id: str, evidence_id: str, reason: str, actor: str) -> str:
        ...
        with self.store.connect() as db:
            existing = db.execute(
                "SELECT request_id FROM resolution_reevaluation_requests WHERE contract_id=? AND evidence_id=?",
                (contract_id, evidence_id),
            ).fetchone()
            if existing:
                return existing["request_id"]
            db.execute(
                "INSERT INTO resolution_reevaluation_requests(request_id,contract_id,evidence_id,reason,status,created_at) VALUES(?,?,?,?,?,?)",
                (request_id, contract_id, evidence_id, reason, "pending", now),
            )
        self.store._audit(
            actor,
            "resolution.reevaluation.requested",
    ...
    def observe(
        ...
        *,
        ...
        normalized_hash = canonical_hash(normalized_value)
        previous = self._latest_evidence(mon["contract_id"], mon["authority_id"])
        if (
            previous
            and previous.get("raw_payload_hash") == raw_hash
            and canonical_hash(previous.get("normalized_value")) == normalized_hash
        ):
        ...
        raw_object = production_data.get_service(self.store).put_json_object(
            raw_payload,
            {
                "kind": "evidence_raw_payload",
                "monitor_id": monitor_id,
                "contract_id": mon["contract_id"],
                "authority_id": mon["authority_id"],
            },
        )
        record = EvidenceRecord(
            evidence_id=gen_id("evid"),
            authority_id=mon["authority_id"],
            authority_version=mon["authority_version"],
            ...
            contract_id=mon["contract_id"],
            effective_at=observed_at or now,
            revision_number=revision_number,
            supersedes=previous.get("evidence_id") if previous else None,
            source_locator=source_locator
            or (self.store.get_authority(mon["authority_id"], mon["authority_version"]) or {}).get("endpoint", ""),
            metadata={
                "monitor_id": monitor_id,
                "adapter_type": mon["adapter_type"],
        ...
        for other in self._latest_other_evidence(mon["contract_id"], mon["authority_id"]):
            if canonical_hash(other.get("normalized_value")) != normalized_hash:
                ...
                exc = self._upsert_exception(
                    ...
                    subject=mon["contract_id"],
                    contract_id=mon["contract_id"],
                    authority_id=mon["authority_id"],
                    severity="critical",
                    title=f"Conflicting governed evidence: {mon['contract_id']}",
                    ...
                    metadata={"evidence_id": saved["evidence_id"], "other_evidence_id": other.get("evidence_id")},
                    actor=actor,
                )
                exception_id = exc["exception_id"]
                break
        reeval = self._request_reevaluation(
            mon["contract_id"], saved["evidence_id"], "governed_evidence_changed", actor
        )
        status = "conflict" if conflict else ("revision" if revision else "appended")
        self._record_poll(
            monitor_id,
            status,
            {"evidence_id": saved["evidence_id"], "revision_number": revision_number, "conflict": conflict},
            attempt=1,
        )
        self._mark_success(
            mon, raw_hash, normalized_hash, saved["evidence_id"], state="conflict" if conflict else "healthy"
        )
    ...
    def _fetch(self, mon: dict[str, Any]) -> tuple[Any, Any, str]:
        ...
        if mon["adapter_type"] == "static":
            if "value" not in cfg:
                raise RuntimeError("static adapter requires config.value")
        ...
        with urllib.request.urlopen(req, timeout=float(cfg.get("timeout_seconds", 10))) as resp:
            ...
            if len(raw_bytes) > int(cfg.get("max_bytes", 1_000_000)):
                raise RuntimeError("source payload exceeds max_bytes")
    ...
    def poll(self, monitor_id: str, actor: str = "system:evidence-worker") -> dict[str, Any]:
        ...
        if not mon:
            raise ValueError("monitor not found")
        started = utcnow()
        attempt = int(mon["consecutive_failures"]) + 1
        try:
            value, raw, locator = self._fetch(mon)
            result = self.observe(
                monitor_id, normalized_value=value, raw_payload=raw, source_locator=locator, actor=actor
            )
            ...
            with self.store.connect() as db:
                db.execute(
                    "UPDATE evidence_monitors SET state='outage',consecutive_failures=?,last_polled_at=?,next_poll_at=?,last_error=?,updated_at=? WHERE monitor_id=?",
            ...
            exc = self._upsert_exception(
                ...
                contract_id=mon["contract_id"],
                authority_id=mon["authority_id"],
                severity="high" if failures < 3 else "critical",
                title=f"Evidence source unavailable: {mon['authority_id']}",
                ...
                metadata={"monitor_id": monitor_id, "consecutive_failures": failures, "next_poll_at": next_poll},
                actor=actor,
            )
            self._record_poll(
                monitor_id,
                "outage",
                {"error": str(e), "consecutive_failures": failures, "next_poll_at": next_poll},
                attempt=attempt,
            )
            self.store._audit(
                actor,
                "evidence.poll.failed",
                "evidence_monitor",
                monitor_id,
    ...
    def _mark_success(
        self, mon: dict[str, Any], raw_hash: str, normalized_hash: str, evidence_id: str | None, state: str
    ) -> None:
        now = utcnow()
        nxt = (_parse_time() + timedelta(seconds=int(mon["schedule_seconds"]))).isoformat()
        with self.store.connect() as db:
            db.execute(
                "UPDATE evidence_monitors SET state=?,consecutive_failures=0,last_polled_at=?,next_poll_at=?,last_success_at=?,last_error=NULL,last_payload_hash=?,last_normalized_hash=?,latest_evidence_id=?,updated_at=? WHERE monitor_id=?",
    ...
    def _record_poll(self, monitor_id: str, status: str, details: dict[str, Any], attempt: int) -> None:
        ...
        with self.store.connect() as db:
            db.execute(
                "INSERT INTO evidence_poll_runs(poll_id,monitor_id,started_at,completed_at,status,attempt,details_json) VALUES(?,?,?,?,?,?,?)",
    ...
    def poll_due(self, limit: int = 50, actor: str = "system:evidence-worker") -> dict[str, Any]:
        ...
        with self.store.connect() as db:
            rows = db.execute(
                "SELECT monitor_id FROM evidence_monitors WHERE enabled=1 AND (next_poll_at IS NULL OR next_poll_at<=?) ORDER BY COALESCE(next_poll_at,created_at) LIMIT ?",
                (now, limit),
            ).fetchall()
    ...
    def list_reevaluations(self, status: str | None = None) -> list[dict[str, Any]]:
        with self.store.connect() as db:
            if status:
                rows = db.execute(
                    "SELECT * FROM resolution_reevaluation_requests WHERE status=? ORDER BY created_at DESC", (status,)
                ).fetchall()
            else:
    ...
    def source_health(self) -> dict[str, Any]:
        ...
        active = [m for m in monitors if m["enabled"]]
        states = {k: sum(1 for m in active if m["state"] == k) for k in {"idle", "healthy", "conflict", "outage"}}
        return {
            "monitors": len(monitors),
            "active": len(active),
            "states": states,
            "healthy": all(m["state"] not in {"outage", "conflict"} for m in active),
            "generated_at": utcnow(),
        }
    ...
    def summary(self) -> dict[str, Any]:
        with self.store.connect() as db:
            ...
            def count(t):
                return int(db.execute(f"SELECT COUNT(*) n FROM {t}").fetchone()["n"])

            return {
                "monitors": count("evidence_monitors"),
                "poll_runs": count("evidence_poll_runs"),
                "exceptions": count("evidence_exceptions"),
                "pending_reevaluations": int(
                    db.execute(
                        "SELECT COUNT(*) n FROM resolution_reevaluation_requests WHERE status='pending'"
                    ).fetchone()["n"]
                ),
