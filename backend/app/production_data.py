...
def load_config() -> DataPlaneConfig:
    ...
    return DataPlaneConfig(
        ...
        database_sslmode=os.environ.get(
            "ARBITER_DATABASE_SSLMODE", "require" if environment in {"prod", "production"} else "prefer"
        ).strip(),
...
def configuration_findings() -> list[dict[str, Any]]:
        ...
        findings.append(
            {
                ...
                "detail": "ARBITER_DATABASE_BACKEND must be sqlite or postgresql.",
            }
        )
    if cfg.production and cfg.database_backend != "postgresql":
        findings.append(
            {
                "severity": "BLOCK",
                "code": "PRODUCTION_DATA_PLANE_NOT_POSTGRESQL",
                "detail": "Production requires PostgreSQL.",
            }
        )
    if cfg.database_backend == "postgresql" and not cfg.database_url:
        findings.append(
            {
                "severity": "BLOCK",
                "code": "POSTGRES_DATABASE_URL_MISSING",
                "detail": "ARBITER_DATABASE_URL is required when PostgreSQL is selected.",
            }
        )
    if cfg.database_backend == "postgresql" and not driver_available("psycopg"):
        findings.append(
            {
                "severity": "BLOCK",
                "code": "PSYCOPG_DRIVER_MISSING",
                "detail": "Install psycopg 3 for the PostgreSQL data plane.",
            }
        )
    if cfg.production and cfg.database_sslmode in {"disable", "allow"}:
        findings.append(
            {
                "severity": "BLOCK",
                "code": "POSTGRES_TLS_NOT_REQUIRED",
                "detail": "Production PostgreSQL connections must require TLS.",
            }
        )
    if cfg.object_store_backend not in {"local", "s3"}:
        findings.append(
            {
                "severity": "BLOCK",
                "code": "UNSUPPORTED_OBJECT_STORE",
                "detail": "ARBITER_OBJECT_STORE_BACKEND must be local or s3.",
            }
        )
    if cfg.production and cfg.object_store_backend != "s3":
        findings.append(
            {
                "severity": "BLOCK",
                "code": "PRODUCTION_OBJECT_STORE_NOT_MANAGED",
                "detail": "Production evidence payloads require managed object storage (s3 adapter in v0.17).",
            }
        )
        ...
        findings.append(
            {
                ...
                "detail": "ARBITER_S3_BUCKET is required for the s3 object store.",
            }
        )
    if cfg.object_store_backend == "s3" and not driver_available("boto3"):
        findings.append(
            {
                "severity": "BLOCK",
                "code": "BOTO3_DRIVER_MISSING",
                "detail": "Install boto3 for the s3 object-store adapter.",
            }
        )
    return findings
...
def apply_postgres_rls(db) -> dict[str, Any]:
    ...
    for table in TENANT_SCOPED_TABLES:
        ...
        if int(nulls["n"]) > 0:
            if not default_tenant:
                raise RuntimeError(
                    f"{table} has pre-v0.17 rows without tenant ownership; set ARBITER_MIGRATION_DEFAULT_TENANT explicitly"
                )
    ...
    db.execute(
        ...
        (
            ...
            hashlib.sha256("|".join(applied).encode()).hexdigest(),
            "production data plane tenant RLS and durability controls",
        ),
    )
...
class LocalObjectStore:
    ...
    def put_bytes(
        self, data: bytes, *, content_type: str = "application/octet-stream", metadata: dict[str, Any] | None = None
    ) -> dict[str, Any]:
        digest = _sha256(data)
...
class S3ObjectStore:
    def __init__(self, bucket: str, prefix: str, kms_key_id: str = ""):
        try:
            ...
            raise RuntimeError("s3 object store selected but boto3 is not installed") from e
        self.bucket = bucket
...
def object_store():
    cfg = load_config()
    if cfg.object_store_backend == "s3":
        return S3ObjectStore(cfg.s3_bucket, cfg.s3_prefix, cfg.s3_kms_key_id)
...
class DataPlaneService:
    ...
    def verify_object(self, ref: dict[str, Any]) -> dict[str, Any]:
        ...
        data = object_store().get_bytes(digest)
        actual = _sha256(data)
    ...
    def create_sqlite_backup(self, actor: str = "system:backup") -> dict[str, Any]:
        ...
        if cfg.database_backend != "sqlite":
            raise RuntimeError("in-process backup is only for the local SQLite reference backend")
        source = Path(self.store.path)
        Path(cfg.backup_dir).mkdir(parents=True, exist_ok=True)
        ...
        manifest = {
            "schema": "arbiter.backup-manifest.v1",
    ...
    def verify_sqlite_backup(self, manifest_file: str) -> dict[str, Any]:
        cfg = load_config()
        mp = Path(cfg.backup_dir) / manifest_file
        manifest = json.loads(mp.read_text())
        bp = Path(cfg.backup_dir) / manifest["backup_file"]
        actual = _sha256(bp.read_bytes())
        ...
        try:
            ...
            con.close()
        return {
            "ok": actual == manifest["sha256"] and integrity == "ok",
            "sha256_ok": actual == manifest["sha256"],
    ...
    def postgres_backup_plan(self) -> dict[str, Any]:
        ...
        return {
            ...
            "requirements": [
                ...
                "checksum/application smoke test after restore",
            ],
    ...
    def self_test(self) -> dict[str, Any]:
        ...
        ref = self.put_json_object(probe, {"purpose": "self-test"})
        verify = self.verify_object(ref)
        translated = _qmark_to_pyformat("SELECT '?' AS literal, value FROM t WHERE a=? AND b=?")
        policy = rls_policy_sql("analysis_cases")
        ctx_before = current_context()
        tokens = bind_context("tenant-self-test", "principal-self-test", "request-self-test")
        try:
            ctx_during = current_context()
        finally:
        ...
        return {
            ...
            "rls": {
                ...
                "statements": policy,
                "force_rls": any("FORCE ROW LEVEL SECURITY" in x for x in policy),
                "with_check": any("WITH CHECK" in x for x in policy),
            },
    ...
    def posture(self) -> dict[str, Any]:
        ...
        return {
            ...
            "object_storage": {
                ...
                else ("SSE-S3" if cfg.object_store_backend == "s3" else "filesystem/local-dev"),
            },
            "backup_restore": self.postgres_backup_plan()
            if cfg.database_backend == "postgresql"
            else {
                "strategy": "SQLite online backup + SHA-256 manifest + integrity_check",
...
def get_service(store):
    ...
    if _SERVICE is None or _SERVICE.store is not store:
        _SERVICE = DataPlaneService(store)
    return _SERVICE
...
def open_database_connection(sqlite_path: str):
    ...
    if cfg.database_backend == "postgresql":
        if not cfg.database_url:
            raise RuntimeError("ARBITER_DATABASE_URL is required for PostgreSQL")
