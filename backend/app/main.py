...
async def enterprise_boundary(request: Request, call_next):
    ...
    findings = (
        enterprise.configuration_findings()
        + production_data.configuration_findings()
        + enterprise_secrets.configuration_findings()
        + identity_federation.posture().get("findings", [])
    )
    if request.url.path.startswith("/api/") and request.url.path not in {
        ...
        if any(f["severity"] == "BLOCK" for f in findings):
            ...
            return JSONResponse(
                status_code=503,
                content={"detail": "unsafe production configuration", "findings": findings},
        ...
        if enterprise.load_runtime_config().require_auth:
            try:
                ...
                principal = identity_tenant.principal_from_auth(
                    auth, production=enterprise.load_runtime_config().production
                )
...
def identity_posture(auth=Depends(developer.require_scope("admin:*"))):
    return identity_tenant.posture(
        resolution_store, enterprise.load_runtime_config(), developer.configured_key_records()
    )


@app.get("/api/tenants", tags=["Enterprise Identity"])
def list_tenants(auth=Depends(developer.require_scope("admin:*"))):
    reg = identity_tenant.TenantRegistry(resolution_store)
    reg.ensure_local()
    return {"tenants": reg.list_tenants()}
...
def create_tenant(inp: TenantCreateIn, auth=Depends(developer.require_scope("admin:*"))):
    ...
    reg.ensure_local()
    try:
        return {"tenant": reg.create_tenant(inp.name, inp.actor, inp.tenant_id, inp.metadata)}
...
def list_principals(tenant_id: str | None = None, auth=Depends(developer.require_scope("admin:*"))):
    reg = identity_tenant.TenantRegistry(resolution_store)
    reg.ensure_local()
    return {"principals": reg.list_principals(tenant_id)}
...
def create_principal(inp: PrincipalCreateIn, auth=Depends(developer.require_scope("admin:*"))):
    ...
    try:
        return {
            "principal": reg.register_principal(
                inp.principal_id, inp.tenant_id, inp.principal_type, inp.display_name, inp.actor, inp.metadata
            )
        }
    except ValueError as e:
...
def configure_model_provider(inp: ModelProviderConfigureIn, auth=Depends(developer.require_scope("admin:*"))):
    ...
    try:
        return {
            "provider": enterprise_secrets.get_service(resolution_store).configure_provider(
                tenant_id=principal.tenant_id,
                provider=inp.provider,
                api_key=inp.api_key,
                default_model=inp.default_model,
                fast_model=inp.fast_model,
                actor=principal.principal_id,
                allowed_purposes=inp.allowed_purposes,
                daily_max_calls=inp.daily_max_calls,
...
def disable_model_provider(provider: str, auth=Depends(developer.require_scope("admin:*"))):
    ...
    try:
        return {
            "provider": enterprise_secrets.get_service(resolution_store).disable_provider(
                principal.tenant_id, provider, principal.principal_id
            )
        }
...
def case_copilot(case_id: str, inp: AIUseIn, auth=Depends(developer.require_scope("ai:use"))):
    ...
    try:
        return model_gateway.get_service(resolution_store).invoke(
            "case_copilot",
            {"case_json": case, "question": inp.question},
            tenant_id=principal.tenant_id,
            principal_id=principal.principal_id,
            actor=principal.principal_id,
            model_tier=inp.model_tier,
...
def create_policy_draft(inp: PolicyDraftIn, auth=Depends(developer.require_scope("policy:write"))):
    try:
        return {
            "draft": governed_policy.get_service(resolution_store).create_draft(
                weights=inp.weights, thresholds=inp.thresholds, actor=inp.actor, note=inp.note
            )
        }
    except ValueError as e:
        raise HTTPException(422, str(e)) from e
...
def approve_policy_draft(
    draft_id: str, inp: GovernedActionIn, auth=Depends(developer.require_scope("approvals:write"))
):
    try:
...
def analyze(inp: AnalyzeIn, auth=Depends(developer.require_scope("cases:write"))):
    pol = policy.load_policy()
    market = {"ticker": "LIVE-INPUT", "title": inp.question, "rules_primary": inp.criteria, "category": "live analysis"}
                    ...
                    "sig": canonical_hash(
                        {
                            "contract": "LIVE-INPUT",
                            "gate": "COMPILER_BLOCK",
                            "unresolved": compilation.get("unresolved_fields", []),
                        }
                    )[:18],
                    "hold": True,
                }
    ...
    boundary = exchange_profiles.integration_boundary(
        inp.exchange_profile, compilation.get("status", ""), resolution.get("outcome", "")
    )
    out = {
        "report": report,
    ...
    saved_case = resolution_store.save_analysis_case(
        ...
        actor=inp.actor,
        case_id=inp.case_id,
        source_template_id=inp.source_template_id,
    )
    out["case"] = saved_case
    if inp.use_llm:
        try:
...
def create_template(inp: TemplateIn, auth=Depends(developer.require_scope("cases:write"))):
    try:
        template = resolution_store.save_template(
            ...
            criteria=inp.criteria,
            actor=inp.actor,
            source_case_id=inp.source_case_id,
            metadata=inp.metadata,
        )
    except Exception as e:
        raise HTTPException(409, str(e)) from e
...
def template_from_case(case_id: str, inp: TemplateFromCaseIn, auth=Depends(developer.require_scope("cases:write"))):
    ...
    try:
        template = resolution_store.save_template(
            ...
            metadata={"compiler_status": case["compiler_status"]},
        )
    except Exception as e:
        raise HTTPException(409, str(e)) from e
    return {"template": template}
        ...
        "top_risks": [
            {
                ...
                "primary_flag": next(
                    (f for k in ("definition", "timing", "source") for f in r["levers"][k]["flags"]), None
                ),
            }
            for r in ranked[:5]
        ],
...
def benchmark_cases(failures_only: bool = False):
    ...
    if failures_only:
        cases = [
            c
            for c in cases
            if (not c.get("validator_pass"))
            or (c.get("expected_failure_mode") != "NONE" and not c.get("detected"))
            or (c.get("expected_failure_mode") == "NONE" and c.get("actual_verdict") != "AUTO-RESOLVE")
        ]
...
def infrastructure_summary():
    return {
        "version": __version__,
        "domain": resolution_store.summary(),
        ...
        "data_plane": production_data.get_service(resolution_store).posture(),
        "enterprise_secrets": enterprise_secrets.get_service(resolution_store).posture(
            production_data.current_context().get("tenant_id") or "local"
        ),
        ...
        "resilience_lab": resilience_lab.get_service(resolution_store).posture(
            production_data.current_context().get("tenant_id") or "local"
        ),
        "external_assurance": external_assurance.get_service(resolution_store).posture(
            production_data.current_context().get("tenant_id") or "local"
        ),
        "shadow_pilot": shadow_pilot.get_service(resolution_store).posture(
            production_data.current_context().get("tenant_id") or "local"
        ),
        "controls": CONTROLS,
        "principle": "Define → Evidence → Resolve → Approve → Authorize → Audit",
    }
...
def append_evidence(inp: EvidenceIn, auth=Depends(developer.require_scope("evidence:write"))):
    record = EvidenceRecord(
        ...
        authority_version=inp.authority_version,
        observed_at=inp.observed_at or utcnow(),
        retrieved_at=inp.retrieved_at or utcnow(),
        normalized_value=inp.normalized_value,
        raw_payload_hash=inp.raw_payload_hash,
        parser_version=inp.parser_version,
        contract_id=inp.contract_id,
        effective_at=inp.effective_at,
        revision_number=inp.revision_number,
        supersedes=inp.supersedes,
        source_locator=inp.source_locator,
        metadata=inp.metadata,
    )
    try:
...
def create_resolution_run(inp: ResolutionRunIn, auth=Depends(developer.require_scope("resolution:write"))):
    ...
    run = ResolutionRun(
        ...
        started_at=utcnow(),
        completed_at=utcnow(),
        exceptions=inp.exceptions,
        approvals=inp.approvals,
        result=inp.result,
    )
    try:
...
def reference_exchange_create_market(inp: ReferenceMarketIn, auth=Depends(developer.require_scope("operations:write"))):
    principal = _principal_from_auth(auth)
    return {
        "market": reference_exchange.get_service(resolution_store).create_market(
            principal.tenant_id, inp.title, inp.rules, principal.principal_id
        )
    }
...
def reference_exchange_get_market(market_id: str, auth=Depends(developer.require_scope("operations:write"))):
    principal = _principal_from_auth(auth)
    try:
        return {"market": reference_exchange.get_service(resolution_store).get_market(market_id, principal.tenant_id)}
    except KeyError as e:
        raise HTTPException(404, str(e)) from e


@app.post("/api/reference-exchange/accounts", tags=["Reference Exchange"])
def reference_exchange_create_account(
    inp: ReferenceAccountIn, auth=Depends(developer.require_scope("operations:write"))
):
    principal = _principal_from_auth(auth)
    return {
        "account": reference_exchange.get_service(resolution_store).create_account(
            principal.tenant_id, inp.display_name, principal.principal_id
        )
    }


@app.post("/api/reference-exchange/positions", tags=["Reference Exchange"])
def reference_exchange_open_position(
    inp: ReferencePositionIn, auth=Depends(developer.require_scope("operations:write"))
):
    principal = _principal_from_auth(auth)
    try:
        return {
            "position": reference_exchange.get_service(resolution_store).place_position(
                principal.tenant_id,
                inp.market_id,
                inp.account_id,
                inp.side,
...
def reference_exchange_resolve(
    market_id: str, inp: ReferenceResolveIn, auth=Depends(developer.require_scope("settlement:authorize"))
):
    principal = _principal_from_auth(auth)
    try:
        return {
            "settlement": reference_exchange.get_service(resolution_store).resolve_market(
                principal.tenant_id, market_id, inp.outcome, inp.evidence_summary, principal.principal_id
            )
        }
    except KeyError as e:
        raise HTTPException(404, str(e)) from e
...
def operations_self_test(auth=Depends(developer.require_scope("admin:*"))):
    principal = _principal_from_auth(auth)
    return operations_resilience.get_service(resolution_store).self_test(principal.tenant_id, principal.principal_id)
...
def operations_incidents(auth=Depends(developer.require_scope("operations:write"))):
    principal = _principal_from_auth(auth)
    return {"incidents": operations_resilience.get_service(resolution_store).list_incidents(principal.tenant_id)}
...
def operations_open_incident(inp: IncidentIn, auth=Depends(developer.require_scope("operations:write"))):
    ...
    try:
        return {
            "incident": operations_resilience.get_service(resolution_store).open_incident(
                principal.tenant_id, inp.severity, inp.title, inp.details, principal.principal_id
            )
        }
    except ValueError as e:
        raise HTTPException(422, str(e)) from e
...
def operations_close_incident(incident_id: str, auth=Depends(developer.require_scope("operations:write"))):
    ...
    try:
        return {
            "incident": operations_resilience.get_service(resolution_store).close_incident(
                principal.tenant_id, incident_id, principal.principal_id
            )
        }
    except KeyError as e:
        raise HTTPException(404, str(e)) from e
...
def operations_recovery_drill(inp: RecoveryDrillIn, auth=Depends(developer.require_scope("operations:write"))):
    ...
    try:
        return {
            "drill": operations_resilience.get_service(resolution_store).run_recovery_drill(
                principal.tenant_id, inp.drill_type, principal.principal_id
            )
        }
    except ValueError as e:
        raise HTTPException(422, str(e)) from e
...
def real_benchmark_run(inp: RealBenchmarkRunIn, auth=Depends(developer.require_scope("benchmark:run"))):
    ...
    result = run_real_benchmark_blind(
        real_benchmark_service.default_dataset_dir(),
        out,
...
def assurance_self_test(auth=Depends(developer.require_scope("admin:*"))):
    principal = _principal_from_auth(auth)
    return settlement_assurance.get_service(resolution_store).self_test(principal.tenant_id, principal.principal_id)
...
def assurance_adversarial(auth=Depends(developer.require_scope("benchmark:run"))):
    principal = _principal_from_auth(auth)
    return settlement_assurance.get_service(resolution_store).run_adversarial_suite(
...
def assurance_holdouts(auth=Depends(developer.require_scope("operations:write"))):
    principal = _principal_from_auth(auth)
    return {"datasets": settlement_assurance.get_service(resolution_store).list_holdouts(principal.tenant_id)}
...
def assurance_create_holdout(inp: HoldoutCreateIn, auth=Depends(developer.require_scope("benchmark:run"))):
    ...
    try:
        return {
            "dataset": settlement_assurance.get_service(resolution_store).create_holdout(
                principal.tenant_id, inp.name, inp.cases, principal.principal_id
            )
        }
    except ValueError as e:
        raise HTTPException(422, str(e)) from e
...
def assurance_evaluate_holdout(dataset_id: str, auth=Depends(developer.require_scope("benchmark:run"))):
    principal = _principal_from_auth(auth)
    try:
        return settlement_assurance.get_service(resolution_store).evaluate_holdout(
            principal.tenant_id, dataset_id, principal.principal_id
        )
    except KeyError as e:
        raise HTTPException(404, str(e)) from e
...
def resilience_lab_posture(auth=Depends(developer.require_scope("operations:write"))):
    principal = _principal_from_auth(auth)
    return resilience_lab.get_service(resolution_store).posture(principal.tenant_id)
...
def resilience_lab_runs(auth=Depends(developer.require_scope("operations:write"))):
    principal = _principal_from_auth(auth)
    return {"runs": resilience_lab.get_service(resolution_store).list_runs(principal.tenant_id)}
...
def resilience_lab_self_test(auth=Depends(developer.require_scope("admin:*"))):
    principal = _principal_from_auth(auth)
    return resilience_lab.get_service(resolution_store).self_test(principal.tenant_id, principal.principal_id)
...
def resilience_lab_run(inp: ResilienceRunIn, auth=Depends(developer.require_scope("benchmark:run"))):
    ...
    try:
        return {
            "run": resilience_lab.get_service(resolution_store).run_local(
                principal.tenant_id, inp.scenario, principal.principal_id, inp.iterations
            )
        }
    except ValueError as e:
        raise HTTPException(422, str(e)) from e
...
def resilience_lab_external(inp: ResilienceExternalIn, auth=Depends(developer.require_scope("admin:*"))):
    ...
    try:
        return {
            "run": resilience_lab.get_service(resolution_store).record_external(
                principal.tenant_id, inp.scenario, inp.environment, inp.status, inp.metrics, principal.principal_id
            )
        }
    except ValueError as e:
        raise HTTPException(422, str(e)) from e
...
def external_assurance_posture(auth=Depends(developer.require_scope("operations:write"))):
    principal = _principal_from_auth(auth)
    return external_assurance.get_service(resolution_store).posture(principal.tenant_id)
...
def external_assurance_list(auth=Depends(developer.require_scope("operations:write"))):
    principal = _principal_from_auth(auth)
    return {"artifacts": external_assurance.get_service(resolution_store).list(principal.tenant_id)}
...
def external_assurance_record(inp: ExternalAssuranceIn, auth=Depends(developer.require_scope("admin:*"))):
    ...
    try:
        return {
            "artifact": external_assurance.get_service(resolution_store).record(
                principal.tenant_id,
                inp.artifact_type,
                inp.provider,
                inp.performed_at,
...
def external_assurance_self_test(auth=Depends(developer.require_scope("admin:*"))):
    principal = _principal_from_auth(auth)
    return external_assurance.get_service(resolution_store).self_test(principal.tenant_id, principal.principal_id)
...
def shadow_pilot_posture(auth=Depends(developer.require_scope("operations:write"))):
    principal = _principal_from_auth(auth)
    return shadow_pilot.get_service(resolution_store).posture(principal.tenant_id)
...
def shadow_pilot_create(inp: ShadowPilotCreateIn, auth=Depends(developer.require_scope("operations:write"))):
    ...
    try:
        return {
            "pilot": shadow_pilot.get_service(resolution_store).create_pilot(
                principal.tenant_id, inp.venue, inp.name, inp.contract_target, principal.principal_id, inp.metadata
            )
        }
    except ValueError as e:
        raise HTTPException(422, str(e)) from e
...
def shadow_pilot_get(pilot_id: str, auth=Depends(developer.require_scope("operations:write"))):
    principal = _principal_from_auth(auth)
    try:
        return {"pilot": shadow_pilot.get_service(resolution_store).get_pilot(principal.tenant_id, pilot_id)}
    except KeyError as e:
        raise HTTPException(404, str(e)) from e


@app.post("/api/shadow-pilots/{pilot_id}/contracts", tags=["Shadow Pilot"])
def shadow_pilot_add_contract(
    pilot_id: str, inp: ShadowContractIn, auth=Depends(developer.require_scope("operations:write"))
):
    principal = _principal_from_auth(auth)
    try:
        return {
            "contract": shadow_pilot.get_service(resolution_store).add_contract(
                principal.tenant_id, pilot_id, inp.venue_contract_id, inp.title, inp.rules, principal.principal_id
            )
        }
...
def shadow_pilot_resolution(
    shadow_contract_id: str, inp: ShadowResolutionIn, auth=Depends(developer.require_scope("resolution:write"))
):
    principal = _principal_from_auth(auth)
    try:
        return {
            "contract": shadow_pilot.get_service(resolution_store).record_resolution(
                principal.tenant_id,
                shadow_contract_id,
                inp.venue_outcome,
                inp.arbiter_outcome,
...
def shadow_pilot_self_test(auth=Depends(developer.require_scope("admin:*"))):
    principal = _principal_from_auth(auth)
    return shadow_pilot.get_service(resolution_store).self_test(principal.tenant_id, principal.principal_id)
...
def reference_exchange_order(inp: ReferenceOrderIn, auth=Depends(developer.require_scope("operations:write"))):
    principal = _principal_from_auth(auth)
    try:
        return reference_exchange.get_service(resolution_store).place_order(
            principal.tenant_id,
            inp.market_id,
            inp.account_id,
            inp.side,
...
def reference_exchange_cancel_order(order_id: str, auth=Depends(developer.require_scope("operations:write"))):
    ...
    try:
        return {
            "order": reference_exchange.get_service(resolution_store).cancel_order(
                principal.tenant_id, order_id, principal.principal_id
            )
        }
    except KeyError as e:
        raise HTTPException(404, str(e)) from e
...
def reference_exchange_order_book(market_id: str, auth=Depends(developer.require_scope("operations:write"))):
    principal = _principal_from_auth(auth)
    return reference_exchange.get_service(resolution_store).order_book(principal.tenant_id, market_id)
...
def reference_exchange_account(account_id: str, auth=Depends(developer.require_scope("operations:write"))):
    principal = _principal_from_auth(auth)
    try:
        return {"account": reference_exchange.get_service(resolution_store).account(account_id, principal.tenant_id)}
    except KeyError as e:
        raise HTTPException(404, str(e)) from e
...
def update_evidence_monitor(
    monitor_id: str, inp: EvidenceMonitorUpdate, auth=Depends(developer.require_scope("evidence:write"))
):
    try:
...
def observe_evidence(
    monitor_id: str, inp: EvidenceObservationIn, auth=Depends(developer.require_scope("evidence:write"))
):
    try:
...
def approvals(status: str | None = None, object_id: str | None = None, limit: int = 100):
    return {
        "approvals": approval_control.get_service(resolution_store).list_approvals(
            status=status, object_id=object_id, limit=min(limit, 500)
        )
    }
...
def decide_approval(
    approval_id: str, inp: ApprovalDecisionIn, auth=Depends(developer.require_scope("approvals:write"))
):
    try:
        return {
            "approval": approval_control.get_service(resolution_store).decide(
                approval_id, actor=inp.actor, decision=inp.decision, note=inp.note
            )
        }
    except ValueError as e:
        raise HTTPException(422, str(e)) from e
...
def settlement_packets(run_id: str | None = None, limit: int = 100):
    return {
        "packets": approval_control.get_service(resolution_store).list_packets(run_id=run_id, limit=min(limit, 500))
    }


@app.post("/api/approvals/{approval_id}/settlement-packet", tags=["Settlement Control"])
def create_settlement_packet(
    approval_id: str, inp: SettlementPacketIn, auth=Depends(developer.require_scope("settlement:authorize"))
):
    try:
        return {
            "packet": approval_control.get_service(resolution_store).create_settlement_packet(
                approval_id, actor=inp.actor
            )
        }
    except ValueError as e:
        raise HTTPException(422, str(e)) from e
...
def list_jobs(
    ...
    tenant_id: str | None = None,
    limit: int = 100,
    auth=Depends(developer.require_scope("operations:write")),
):
    return {
        "jobs": reliability.get_service(resolution_store).list_jobs(
            status=status, tenant_id=tenant_id, limit=min(limit, 500)
        )
    }
...
def data_plane_backup(auth=Depends(developer.require_scope("admin:*"))):
    ...
    if production_data.load_config().database_backend == "postgresql":
        return {
            "executed": False,
            "plan": service.postgres_backup_plan(),
            "note": "Managed PostgreSQL backups/PITR must run in deployment infrastructure; Arbiter exposes the required recovery posture but does not dump a live production database through the API.",
        }
