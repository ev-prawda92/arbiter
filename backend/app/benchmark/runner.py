...
def _scores(report: dict) -> dict[str, int]:
    return {
        lever: report.get("levers", {}).get(lever, {}).get("score", 0) for lever in ("source", "timing", "definition")
    }
...
def run(engine_module, policy: dict, dataset_path: str | Path) -> dict:
    ...
    for case in cases:
        ...
        report = (
            parents[case.id] if case.expected.failure_mode == "NONE" else engine_module.analyze(_market(case), policy)
        )
        ...
        rows.append(
            {
                ...
                "kind": case.kind,
                "expected_lever": expected_lever,
                "expected_failure_mode": case.expected.failure_mode,
                "validator_pass": ok,
                "validator_note": note,
                "actual_verdict": report.get("verdict", {}).get("label"),
                "actual_composite": report.get("composite"),
                "lever_scores": _scores(report),
                "lever_deltas_vs_parent": deltas,
                "detected": detected,
