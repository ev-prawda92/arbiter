...
def load_config() -> OIDCConfig:
    ...
    enabled = os.environ.get("ARBITER_OIDC_ENABLED", "false").lower() in {"1", "true", "yes"} or bool(
        issuer and audience and jwks
    )
    return OIDCConfig(
        enabled,
        issuer,
        audience,
        jwks,
        os.environ.get("ARBITER_OIDC_TENANT_CLAIM", "tenant_id"),
        os.environ.get("ARBITER_OIDC_PRINCIPAL_CLAIM", "sub"),
        os.environ.get("ARBITER_OIDC_SCOPE_CLAIM", "scope"),
        tuple(x.strip() for x in os.environ.get("ARBITER_OIDC_ALGORITHMS", "RS256").split(",") if x.strip()),
    )


def _scopes(claim: Any) -> list[str]:
    if isinstance(claim, str):
        return [x for x in claim.split() if x]
...
def map_claims(claims: dict[str, Any], cfg: OIDCConfig | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "mode": "oidc-bearer",
        "key_id": None,
        "scopes": _scopes(claims.get(cfg.scope_claim)),
        "tenant_id": tenant,
...
def authenticate_bearer(token: str) -> dict[str, Any]:
    cfg = load_config()
    if not cfg.enabled:
        raise ValueError("OIDC federation is not configured")
    if not (cfg.issuer and cfg.audience and cfg.jwks_url):
...
def posture() -> dict[str, Any]:
    cfg = load_config()
    production = os.environ.get("ARBITER_ENV", "local").lower() == "production"
    findings = []
    if cfg.enabled and not (cfg.issuer and cfg.audience and cfg.jwks_url):
        findings.append(
            {
                "severity": "BLOCK" if production else "WARN",
                "code": "OIDC_CONFIG_INCOMPLETE",
                "detail": "OIDC requires issuer, audience, and JWKS URL.",
            }
        )
    return {
        "version": VERSION,
        ...
        "jwks_configured": bool(cfg.jwks_url),
        "tenant_claim": cfg.tenant_claim,
        "principal_claim": cfg.principal_claim,
        "scope_claim": cfg.scope_claim,
        "algorithms": list(cfg.algorithms),
        "findings": findings,
    }
...
def self_test() -> dict[str, Any]:
    ...
    return {
        "ok": mapped["principal_id"] == "user:test"
        and mapped["tenant_id"] == "tenant:test"
        and "ai:use" in mapped["scopes"],
        "mapped": mapped,
        "signature_call_performed": False,
    }
