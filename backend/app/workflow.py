...
from . import operations_intelligence
...
def build_work_queue(
    ...
    # Active Evidence v0.11 exceptions are first-class operator work. They are
    # generated by governed source monitoring, not by an advisory model.
    for exc in infrastructure.get("evidence_exceptions", []) or []:
        add(
            exc.get("kind", "evidence_exception"),
            exc.get("subject", exc.get("exception_id", "unknown")),
            exc.get("title", "Evidence exception"),
            exc.get("detail", "Governed evidence requires review."),
...
def build_agent_brief(queue: dict, executive: dict) -> dict[str, Any]:
    ...
    v0.31 also attaches deterministic Operations Intelligence: queue compression,
    blocker clustering, readiness, and a recommended work sequence. A language
    model may later explain these facts, but the briefing itself remains fully
    traceable and non-binding.
    """
    ...
    ops = operations_intelligence.analyze_queue(queue, executive)
    ops_summary = ops.get("summary", {})

    if not active:
        headline = "No active resolution exceptions require operator attention."
    else:
        patterns = int(ops_summary.get("distinct_work_patterns", len(active)) or 0)
        ready = int(ops_summary.get("ready_for_review", 0) or 0)
        waiting = int(ops_summary.get("waiting_on_external_data", 0) or 0)
        headline = f"{len(active)} active cases compress into {patterns} work pattern(s); {ready} are ready now and {waiting} are waiting on external data."
