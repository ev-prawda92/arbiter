        ...
        with self._lock:
            ...
            if len(q) >= limit:
                return {
                    "admitted": False,
                    "remaining": 0,
                    "retry_after_seconds": max(1, int(window_seconds - (now - q[0]))),
                }
            q.append(now)
...
class OperationsService:
    ...
    def dependency_status(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            deps.append({"name": "database", "status": "unhealthy", "required": True, "error": type(e).__name__})
        audit = self.store.verify_audit_chain()
        deps.append({"name": "audit_chain", "status": "healthy" if audit.get("ok") else "unhealthy", "required": True})
        return deps

    def posture(self) -> dict[str, Any]:
        deps = self.dependency_status()
        required_ok = all(d["status"] == "healthy" for d in deps if d["required"])
        with self.store.connect() as db:
            open_inc = int(
                db.execute("SELECT COUNT(*) n FROM operations_incidents WHERE status!='closed'").fetchone()["n"]
            )
            drills = int(db.execute("SELECT COUNT(*) n FROM recovery_drills").fetchone()["n"])
            try:
                backlog = int(
                    db.execute(
                        "SELECT COUNT(*) n FROM durable_jobs WHERE status IN ('queued','leased','running')"
                    ).fetchone()["n"]
                )
            except Exception:
                backlog = 0
        ...
        pressure = (
            "BLOCK"
            if backlog >= cfg["worker_backlog_block"]
            else ("WARN" if backlog >= cfg["worker_backlog_warn"] else "NORMAL")
        )
        return {
            "version": "0.20.0",
            "ready": required_ok and pressure != "BLOCK",
            "dependencies": deps,
            "backlog": backlog,
            "backpressure": pressure,
            "open_incidents": open_inc,
            "recovery_drills": drills,
            "rate_limiting": "sliding-window-admission",
            "config": cfg,
            "external_observability_required_for_production": True,
        }
    ...
    def open_incident(self, tenant_id: str, severity: str, title: str, details: dict, actor: str):
        ...
        with self.store.connect() as db:
            db.execute(
                "INSERT INTO operations_incidents(incident_id,tenant_id,severity,title,status,opened_at,opened_by,details_json) VALUES(?,?,?,?,?,?,?,?)",
                (iid, tenant_id, severity, title, "open", now, actor, json.dumps(details, sort_keys=True)),
            )
        self.store._audit(
            actor, "incident.opened", "incident", iid, {"tenant_id": tenant_id, "severity": severity, "title": title}
        )
    ...
    def close_incident(self, tenant_id: str, incident_id: str, actor: str):
        ...
        with self.store.connect() as db:
            row = db.execute(
                "SELECT * FROM operations_incidents WHERE incident_id=? AND tenant_id=?", (incident_id, tenant_id)
            ).fetchone()
            if not row:
                raise KeyError("unknown incident")
            db.execute(
                "UPDATE operations_incidents SET status='closed',closed_at=?,closed_by=? WHERE incident_id=?",
    ...
    def list_incidents(self, tenant_id: str):
        with self.store.connect() as db:
            rows = db.execute(
                "SELECT * FROM operations_incidents WHERE tenant_id=? ORDER BY opened_at DESC", (tenant_id,)
            ).fetchall()
    ...
    def run_recovery_drill(self, tenant_id: str, drill_type: str, actor: str):
        if drill_type not in {"audit-replay", "database-read", "queue-recovery", "backup-posture"}:
            raise ValueError("unsupported drill type")
        start = utcnow()
        details = {"drill_type": drill_type}
        if drill_type == "audit-replay":
            details["audit_chain"] = self.store.verify_audit_chain()
            ok = details["audit_chain"].get("ok") is True
        elif drill_type == "database-read":
            try:
                with self.store.connect() as db:
                    v = db.execute("SELECT 1 AS ok").fetchone()["ok"]
                details["database_read"] = v
                ok = v == 1
            except Exception as e:
            ...
            try:
                with self.store.connect() as db:
                    n = db.execute("SELECT COUNT(*) n FROM durable_jobs").fetchone()["n"]
                details["jobs_visible"] = int(n)
                ok = True
            except Exception as e:
                details["error"] = type(e).__name__
                ok = False
            ...
            details["backup_posture"] = production_data.get_service(self.store).posture().get("backup_restore")
            ok = bool(details["backup_posture"])
        end = utcnow()
        ...
        rh = canonical_hash(payload)
        did = gen_id("drill")
        with self.store.connect() as db:
            db.execute(
                "INSERT INTO recovery_drills(drill_id,tenant_id,drill_type,started_at,completed_at,result,details_json,result_hash,actor) VALUES(?,?,?,?,?,?,?,?,?)",
                (did, tenant_id, drill_type, start, end, result, json.dumps(details, sort_keys=True), rh, actor),
            )
        self.store._audit(
            actor,
            "recovery_drill.completed",
            "recovery_drill",
            did,
            {"tenant_id": tenant_id, "drill_type": drill_type, "result": result, "result_hash": rh},
        )
    ...
    def self_test(self, tenant_id="local", actor="system:v0.20-self-test"):
        admission = self.admit(tenant_id, actor)
        d1 = self.run_recovery_drill(tenant_id, "audit-replay", actor)
        d2 = self.run_recovery_drill(tenant_id, "database-read", actor)
        inc = self.open_incident(tenant_id, "SEV4", "Self-test incident", {"synthetic": True}, actor)
        closed = self.close_incident(tenant_id, inc["incident_id"], actor)
        return {
...
def get_service(store):
    ...
    if _SERVICE is None or _SERVICE.store is not store:
        _SERVICE = OperationsService(store)
    return _SERVICE
