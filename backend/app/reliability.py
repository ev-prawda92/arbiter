...
class ReliabilityService:
    ...
    def idempotency_put(
        self, key: str, scope: str, request: Any, response: Any, status_code: int = 200, ttl_seconds: int = 86400
    ) -> dict:
        now = _dt()
    ...
    def enqueue_job(
        ...
        self.store._audit(
            ...
            "job",
            job_id,
            {"job_type": job_type, "tenant_id": tenant_id, "payload_hash": payload_hash},
        )
    ...
    def claim_job(self, worker_id: str, tenant_id: str | None = None, lease_seconds: int = 60) -> dict | None:
        ...
        now = _dt()
        stale = _iso(now - timedelta(seconds=max(5, lease_seconds)))
        with self.store.connect() as db:
            # Requeue expired leases first.
            db.execute(
                "UPDATE durable_jobs SET status='queued',locked_at=NULL,locked_by=NULL,updated_at=? WHERE status='running' AND locked_at<?",
                (_iso(now), stale),
            )
            if tenant_id:
                row = db.execute(
                    "SELECT job_id FROM durable_jobs WHERE status='queued' AND tenant_id=? AND available_at<=? ORDER BY available_at,created_at LIMIT 1",
                    (tenant_id, _iso(now)),
                ).fetchone()
            else:
                row = db.execute(
                    "SELECT job_id FROM durable_jobs WHERE status='queued' AND available_at<=? ORDER BY available_at,created_at LIMIT 1",
                    (_iso(now),),
                ).fetchone()
            if not row:
                return None
            job_id = row["job_id"]
            cur = db.execute(
                "UPDATE durable_jobs SET status='running',attempts=attempts+1,locked_at=?,locked_by=?,updated_at=? WHERE job_id=? AND status='queued'",
                (_iso(now), worker_id, _iso(now), job_id),
            )
            if cur.rowcount != 1:
                return None
    ...
    def complete_job(self, job_id: str, worker_id: str, result: dict | None = None) -> dict:
        ...
        with self.store.connect() as db:
            ...
            if row["locked_by"] != worker_id:
                raise PermissionError("job lease belongs to another worker")
            db.execute(
                "UPDATE durable_jobs SET status='completed',completed_at=?,updated_at=?,locked_at=NULL,locked_by=NULL,last_error=NULL WHERE job_id=?",
                (now, now, job_id),
            )
    ...
    def fail_job(self, job_id: str, worker_id: str, error: str, retry_delay_seconds: int = 30) -> dict:
        ...
        with self.store.connect() as db:
            ...
            db.execute(
                "UPDATE durable_jobs SET status=?,available_at=?,updated_at=?,locked_at=NULL,locked_by=NULL,last_error=? WHERE job_id=?",
                (status, available, _iso(now), error[:2000], job_id),
            )
        self.store._audit(
            worker_id, "job.dead_lettered" if terminal else "job.retry_scheduled", "job", job_id, {"error": error[:500]}
        )
        return self.get_job(job_id)
    ...
    def _signing_secret() -> str:
        return os.environ.get("ARBITER_WEBHOOK_SIGNING_SECRET") or (
            "dev-webhook-secret"
            if os.environ.get("ARBITER_ENV", "development").lower() not in {"prod", "production"}
            else ""
        )
    ...
    def create_webhook(
        ...
        if not target_url.startswith("https://") and os.environ.get("ARBITER_ENV", "development").lower() in {
            "prod",
            "production",
        }:
        ...
        delivery_id = gen_id("wh")
        payload_hash = canonical_hash(payload)
        message = f"{now}.{nonce}.{payload_hash}".encode()
        sig = "sha256=" + hmac.new(secret.encode(), message, hashlib.sha256).hexdigest()
        with self.store.connect() as db:
            db.execute(
                "INSERT INTO webhook_deliveries(delivery_id,tenant_id,event_type,target_url,payload_json,payload_hash,nonce,timestamp,signature,status,attempts,max_attempts,next_attempt_at,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                (
                    delivery_id,
                    tenant_id,
                    event_type,
        ...
        self.store._audit(
            ...
            "webhook_delivery",
            delivery_id,
            {"event_type": event_type, "target_url": target_url, "payload_hash": payload_hash},
        )
        return self.get_webhook(delivery_id)

    def get_webhook(self, delivery_id: str) -> dict | None:
        with self.store.connect() as db:
            row = db.execute("SELECT * FROM webhook_deliveries WHERE delivery_id=?", (delivery_id,)).fetchone()
        if not row:
            return None
        d = dict(row)
        d["payload"] = json.loads(d.pop("payload_json"))
    ...
    def verify_webhook(
        ...
        try:
            age = abs((_dt() - _dt(timestamp)).total_seconds())
        except Exception:
            return {"valid": False, "reason": "invalid timestamp"}
        if age > tolerance_seconds:
            return {"valid": False, "reason": "timestamp outside tolerance"}
        payload_hash = canonical_hash(payload)
        msg = f"{timestamp}.{nonce}.{payload_hash}".encode()
        expected = "sha256=" + hmac.new(secret.encode(), msg, hashlib.sha256).hexdigest()
        if not hmac.compare_digest(expected, signature):
            return {"valid": False, "reason": "signature mismatch"}
        if consume_nonce:
            now = _dt()
            exp = _iso(now + timedelta(seconds=tolerance_seconds))
            with self.store.connect() as db:
                db.execute("DELETE FROM replay_nonces WHERE expires_at<=?", (_iso(now),))
                if db.execute("SELECT 1 FROM replay_nonces WHERE nonce=?", (nonce,)).fetchone():
                    return {"valid": False, "reason": "replay detected"}
                db.execute(
                    "INSERT INTO replay_nonces(nonce,first_seen_at,expires_at) VALUES(?,?,?)", (nonce, _iso(now), exp)
                )
    ...
    def posture(self) -> dict:
        with self.store.connect() as db:
            ...
            for status in ("queued", "running", "completed", "dead_letter"):
                counts[status] = int(
                    db.execute("SELECT COUNT(*) n FROM durable_jobs WHERE status=?", (status,)).fetchone()["n"]
                )
            pending = int(
                db.execute("SELECT COUNT(*) n FROM webhook_deliveries WHERE status IN ('pending','retry')").fetchone()[
                    "n"
                ]
            )
            ...
            findings.append(
                {
                    ...
                    "detail": "Production webhook envelopes require ARBITER_WEBHOOK_SIGNING_SECRET or KMS/HSM-backed signing.",
                }
            )
        if prod and os.environ.get("ARBITER_DATABASE_BACKEND", "sqlite").lower() != "postgresql":
            findings.append(
                {
                    "severity": "BLOCK",
                    "code": "RELIABILITY_STORE_NOT_POSTGRESQL",
                    "detail": "Production reliability state must use PostgreSQL/managed durable infrastructure.",
                }
            )
        return {
            "version": "0.15.0",
            ...
            "findings": findings,
            "boundary": "Durability primitives are implemented. Production still requires managed PostgreSQL/queue infrastructure, HA deployment, external observability, and independent failure testing.",
        }
